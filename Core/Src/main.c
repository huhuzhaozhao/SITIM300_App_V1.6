...
SENSOR_DATA sensor_data = { 0.0F };	// 融合后传感器数据
IMU_ARRAY imu_array = { 0.0F };		// 8组IMU数据
LP_FILTER filter[D3] = { 0.0F };	// 3轴陀螺低通滤波
...
int main(void)
{
  ...
  MX_GPIO_Init();
  MX_DMA_Init();
	read_cs_data((char*)(0x08060000),0x0C);//bd
	STMFLASH_Read(ADDR_FLASH_SECTOR_2_BANK1,&ul_CmdData,1);
  MX_SPI2_Init();
  MX_SPI3_Init();
  MX_SPI4_Init();
  MX_SPI6_Init();
  ...
  while (1)
  {
      ...
      TimeEnd(watch0);     
      imuProcess(); 
			if(0x010A == ul_CmdData)
			Frame_Protocol_Packet_Send1();
			else if(0x020A == ul_CmdData)
			Frame_Protocol_Packet_Send3();
			else if(0x030A == ul_CmdData)
			Frame_Protocol_Packet_Send2();		
		}
